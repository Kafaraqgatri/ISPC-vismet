%% Code from mikexcohen@gmail.com to accomany Lecturelet on permutation testing
...
%% Choose channels, and setup wavelet parameters
...
n_permutes = 1000;
n_display_update = round(n_permutes/10); % update every 10%
...
disp(['Starting permutation testing with ' num2str(n_permutes) ' permutations']);
fprintf('%s','...');

for permi = 1:n_permutes
    
    % update display
    if mod(permi,n_display_update) == 0
        fprintf('%s',[num2str(permi) '...']);
    end
...
% Return cursor to new line in command display window
fprintf('\n');
...
subplot(224) % Mean perm map
imagesc(times2save,frex,mean_h0);
hold on
xlabel('Time (ms)'), ylabel('Frequency (Hz)')
set(gca,'clim',[-mean(clim) mean(clim)],'xlim',xlim,'ydir','norm')
title('Mean map across all permutations');
...
%% show histogram of maximum cluster sizes
